...
public final class PairingListener: @unchecked Sendable {
    ...
    /// window from in here cannot truncate the frame it is reacting to. The consumer's job is
    ...
    /// path fires `onPaired` from the sealed frame's own send completion, which is what makes
    /// a consumer that calls `stop()` the instant it pairs safe rather than lucky.
    public func stop() {
        dispatchPrecondition(condition: .onQueue(queue))
    ...
    private func reply(
        ...
        FleetSocket.send(frame, over: connection, onSent: { [weak self] in self?.drop(id) })
    }
