// Adapted from ghostty v1.3.1: macos/Sources/Ghostty/Surface View/SurfaceView_AppKit.swift
import AppKit
import Combine
import SwiftUI
import CoreText
...
extension Ghostty {
    ...
    // Flight Deck: dropped `Codable` conformance (window/session restoration used
    // the global `AppDelegate.ghostty.app`; out of scope for the single-surface skeleton).
    class SurfaceView: OSView, ObservableObject, Identifiable {
        ...
        @objc private func ghosttyConfigDidChange(_ notification: SwiftUI.Notification) {
            // Flight Deck: skeleton has no live `Ghostty.Config` to re-derive from,
            // so config-change notifications are a no-op here.
        }
        ...
        override func mouseMoved(with event: NSEvent) {
            ...
            // Flight Deck: dropped focus-follows-mouse (needed the app-shell
            // `BaseTerminalController`); the single-surface skeleton is always focused.
        }
        ...
        override func menu(for event: NSEvent) -> NSMenu? {
            ...
            menu.addItem(.separator())
            // Flight Deck: dropped "Change Tab Title..." (targeted the app-shell
            // `BaseTerminalController`); no tabs in the single-surface skeleton.
        ...
        // Flight Deck: simplified. The upstream struct also carried
        // `backgroundBlur: Ghostty.Config.BackgroundBlur` and
        // `scrollbar: Ghostty.Config.Scrollbar` plus an `init(_ config: Ghostty.Config)`;
        // those pulled in the 900-line app config type and were only read by the
        // (dropped) SwiftUI scroll-view wrapper. Kept the default-only shape.
        ...
        // Flight Deck: dropped `Codable` (`init(from:)` / `encode(to:)` / `CodingKeys`).
        // Decoding required the global `AppDelegate.ghostty.app` and threw
        // `TerminalRestoreError`; session restoration is out of scope here.
    }
}
